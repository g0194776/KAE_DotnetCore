...
namespace KJFramework.ApplicationEngine
{
    ...
    internal static class APPExecuter
    {
        ...
        private static string DownloadAPP(string remoteAPPPath)
        {
            ...
            try
            {
                string tmpPath = SystemWorker.ConfigurationProxy.GetField("kae-system", "default-app-download-storage-path", false);
            ...
            using (StreamWriter writer = new StreamWriter(new FileStream(destAPPPath, FileMode.Create), Encoding.UTF8))
            {
                writer.Write(client.GetByteArrayAsync(remoteAPPPath).Result);
                writer.Flush();
            }
            return destAPPPath;
        }
        ...
        private static void HandleSystemCommand(IMessageTransaction<MetadataContainer> transaction)
        {
            ...
            lock (_commandExecLockObj)
            {
                ...
                if (result.ErrorId != (byte)KAEErrorCodes.OK)
                    rspMsg.SetAttribute(0x0B, new StringValueStored(result.Error));
