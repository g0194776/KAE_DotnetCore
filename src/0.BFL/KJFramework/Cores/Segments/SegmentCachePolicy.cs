...
namespace KJFramework.Cores.Segments
{
    ...
    public class SegmentCachePolicy : ISegmentCachePolicy
    {
        ...
        private readonly List<SegmentSizePair> _pairs = new List<SegmentSizePair>();
        /// <summary>
        ///     获取片段分布等级
        /// </summary>
        public int SegmentLevel => _segmentLevel;
        ...
        public void Set(int size) => Set(size, 1 - _currentPercent);
        ...
        {
            if (size <= 0) throw new OutOfRangeException("Illegal cache size.");
            if (percent <= 0 || percent > 1) throw new OutOfRangeException("Illegal cache percent.");
            if (_currentPercent == 1 || (_currentPercent + percent) > 1) throw new OutOfRangeException("Cannot set a segment cache policy ,because current percent is full(100%).");
        ...
        public List<SegmentSizePair> Get() => _pairs;
