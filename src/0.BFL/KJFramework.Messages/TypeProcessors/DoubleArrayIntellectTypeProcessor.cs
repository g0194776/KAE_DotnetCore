...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    ///     Double数组类型处理器
    /// </summary>
    public class DoubleArrayIntellectTypeProcessor : IntellectTypeProcessor
    {
        #region Constructor
        ...
        ///     Double数组类型处理器
        /// </summary>
        public DoubleArrayIntellectTypeProcessor()
        {
        ...
        {
            double[] value = analyseResult.GetValue<double[]>(target);
            if (value == null)
            {
            ...
            if (value.Length > 10)
            {
                unsafe
                {
                    fixed (double* pByte = value) proxy.WriteMemory(pByte, (uint) (value.Length*Size.Double));
                }
            }
            else for (int i = 0; i < value.Length; i++) proxy.WriteDouble(value[i]);
            proxy.WriteBackInt32(position, (int) (value.Length*Size.Double + 4));
        }
        ...
        public unsafe override void Process(IMemorySegmentProxy proxy, object target, bool isArrayElement = false, bool isNullable = false)
        {
            ...
            if (array.Length > 10) fixed (double* pByte = array) proxy.WriteMemory(pByte, (uint)array.Length * Size.Double);
            else for (int i = 0; i < array.Length; i++) proxy.WriteDouble(array[i]);
        }
        ...
        public override void Process(object instance, GetObjectAnalyseResult result, byte[] data, int offset, int length = 0)
        {
            if (length == 4)
            {
                result.SetValue(instance, new double[0]);
                return;
            }
            double[] array;
            unsafe
            {
                fixed (byte* pByte = &data[offset])
                {
                    ...
                    if (arrLength > 10)
                    {
                        fixed (double* point = array)
                        {
                            Buffer.MemoryCopy((void*)new IntPtr(pByte + 4), (void*)new IntPtr((byte*)point), (uint)(Size.Double * arrLength), (uint)(Size.Double * arrLength));
                            //Native.Win32API.memcpy(new IntPtr((byte*)point), new IntPtr(pByte + 4), (uint)(Size.Double * arrLength));
                        }
                    }
                    else
                    {
                        double* point = (double*)(pByte + 4);
