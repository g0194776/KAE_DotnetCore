...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    ///     DateTime数组类型处理器
    /// </summary>
    public class DateTimeArrayIntellectTypeProcessor : IntellectTypeProcessor
    {
        #region Constructor
        ...
        ///     DateTime数组类型处理器
        /// </summary>
        public DateTimeArrayIntellectTypeProcessor()
        {
        ...
        {
            DateTime[] value = analyseResult.GetValue<DateTime[]>(target);
            if (value == null)
            {
            ...
            if (value.Length > 10)
            {
                unsafe
                {
                    fixed (DateTime* pByte = value) proxy.WriteMemory(pByte, (uint) value.Length*Size.DateTime);
                }
            }
            else for (int i = 0; i < value.Length; i++) proxy.WriteDateTime(value[i]);
            proxy.WriteBackInt32(position, (int) (value.Length*Size.DateTime + 4));
        }
        ...
        {
            DateTime[] array = (DateTime[])target;
            if (array == null || array.Length == 0) return;
            if (array.Length > 10) fixed (DateTime* pByte = array) proxy.WriteMemory(pByte, (uint)array.Length * Size.DateTime);
            else for (int i = 0; i < array.Length; i++) proxy.WriteDateTime(array[i]);
        }
