...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    public class SByteArrayIntellectTypeProcessor : IntellectTypeProcessor
    {
        ...
        public unsafe override void Process(IMemorySegmentProxy proxy, object target, bool isArrayElement = false, bool isNullable = false)
        {
            ...
            if (array.Length > 10) fixed (sbyte* pByte = array) proxy.WriteMemory(pByte, (uint)array.Length * Size.SByte);
            else for (int i = 0; i < array.Length; i++) proxy.WriteSByte(array[i]);
        }
            ...
            unsafe
            {
                fixed (byte* pByte = &data[offset])
                {
                    ...
                    if (arrLength > 10)
                    {
                        ...
                        {
                            Buffer.MemoryCopy((void*)new IntPtr(pByte + 4), (void*)new IntPtr((byte*)point), (uint)(Size.SByte * arrLength), (uint)(Size.SByte * arrLength));
