...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    ///     Decimal数组类型处理器
    /// </summary>
    public class DecimalArrayIntellectTypeProcessor : IntellectTypeProcessor
    {
        #region Constructor
        ...
        ///     Decimal数组类型处理器
        /// </summary>
        public DecimalArrayIntellectTypeProcessor()
        {
            _supportedType = typeof(decimal[]);
        ...
        public override void Process(IMemorySegmentProxy proxy, IntellectPropertyAttribute attribute, ToBytesAnalyseResult analyseResult, object target, bool isArrayElement = false, bool isNullable = false)
        {
            ...
            else for (int i = 0; i < value.Length; i++) proxy.WriteDecimal(value[i]);
        ...
        public unsafe override void Process(IMemorySegmentProxy proxy, object target, bool isArrayElement = false, bool isNullable = false)
        {
            ...
            if (array.Length > 10) fixed (decimal* pByte = array) proxy.WriteMemory(pByte, (uint)array.Length * Size.Decimal);
            else for (int i = 0; i < array.Length; i++) proxy.WriteDecimal(array[i]);
        }
