...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    ///     Float数组类型处理器
    /// </summary>
    public class FloatArrayIntellectTypeProcessor : IntellectTypeProcessor
    {
        #region Constructor
        ...
        ///     Float数组类型处理器
        /// </summary>
        public FloatArrayIntellectTypeProcessor()
        {
        ...
        {
            float[] value = analyseResult.GetValue<float[]>(target);
            if (value == null)
            {
            ...
            if (value.Length > 10)
            {
                unsafe
                {
                    fixed (float* pByte = value) proxy.WriteMemory(pByte, (uint) (value.Length*Size.Float));
                }
            }
            else for (int i = 0; i < value.Length; i++) proxy.WriteFloat(value[i]);
            proxy.WriteBackInt32(position, (int) (value.Length*Size.Float + 4));
        }
        ...
        public unsafe override void Process(IMemorySegmentProxy proxy, object target, bool isArrayElement = false, bool isNullable = false)
        {
            ...
            if (array.Length > 10) fixed (float* pByte = array) proxy.WriteMemory(pByte, (uint)array.Length * Size.Float);
            else for (int i = 0; i < array.Length; i++) proxy.WriteFloat(array[i]);
        }
        ...
        public override void Process(object instance, GetObjectAnalyseResult result, byte[] data, int offset, int length = 0)
        {
            if (length == 4)
            {
                result.SetValue(instance, new float[0]);
                return;
            }
            float[] array;
            unsafe
            {
                fixed (byte* pByte = &data[offset])
                {
                    ...
                    array = new float[arrLength];
                    if (arrLength > 10)
                    {
                        fixed (float* point = array)
                        {
                            Buffer.MemoryCopy((void*)new IntPtr(pByte + 4), (void*)new IntPtr((byte*)point), (uint)(Size.Float * arrLength), (uint)(Size.Float * arrLength));
                            //Native.Win32API.memcpy(new IntPtr((byte*)point), new IntPtr(pByte + 4), (uint)(Size.Float * arrLength));
                        }
                    }
                    else
                    {
                        float* point = (float*)(pByte + 4);
