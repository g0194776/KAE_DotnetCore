...
namespace KJFramework.Messages.Stubs
{
    ...
    internal sealed class PropertyStubHelper
    {
        ...
        public static IPropertyStub Create(Type targetType, Type outputType, MethodInfo method)
        {
            ...
            MethodInfo invokeMethod = funcType.GetTypeInfo().GetMethod("Invoke");
            MethodInfo createMethod = typeof(Delegate).GetTypeInfo().GetMethod("CreateDelegate", new[] { typeof(Type), typeof(MethodInfo) });
            AssemblyBuilder assBuilder = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("assembly.New"), AssemblyBuilderAccess.Run);
            ...
            methodBuilder.SetReturnType(genericTypeParameterBuilders[0].AsType());
            ...
            #endregion
            return (IPropertyStub)typeBuilder.Assembly.CreateInstance(typeBuilder.CreateTypeInfo().FullName);
        }
