...
using KJFramework.Architecture.UnitTests;
...
namespace KJFramework.Net.Channels.UnitTest
{
    ...
    public class TcpChannelTest
    {
        ...
        {
            SystemWorker.Initialize("KAEWorker", RemoteConfigurationSetting.Default, new FakedRemoteConfigurationProxy());
        }
        ...
        public void SendMessageTest()
        {
            ...
            transportChannel.Disconnect();
            if (!resetEvent.WaitOne(10000)) throw new System.Exception("#Cannot passed transport channel disconnect test.");
            Assert.IsTrue(transportChannel.CommunicationState == CommunicationStates.Closed);
            Thread.Sleep(2000);
            Assert.IsTrue(!connectedChannel.IsConnected);
            Assert.IsTrue(hostChannel.UnRegist());
