...
namespace KJFramework.Containers
{
    ...
    public class SegmentCacheContainer<T> : ISegmentCacheContainer<T>
    {
        ...
        /// <exception cref="OutOfRangeException">非法的参数取值范围</exception>
        /// <exception cref="ArgumentNullException">参数不能为空</exception>
        public SegmentCacheContainer(int capacity, ISegmentCachePolicy policy)
        {
            if (policy == null) throw new ArgumentNullException(nameof(policy));
            if (capacity <=0) throw new OutOfRangeException("Illegal capacity.");
            Capacity = capacity;
            _policy = policy;
        ...
        {
            if (_policy.SegmentLevel <= 0) throw new OutOfRangeException("Cannot initialize a segment cache container, because the segment level must larger than zero.");
                ...
                while (true)
                {
                    lock (_lockObj)
                    {
                        ...
                        foreach (KeyValuePair<T, IHightSpeedSegmentCache> pair in _caches)
                            if (pair.Value.IsDead) values.Add(pair.Key);
